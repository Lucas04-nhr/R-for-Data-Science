...
income_pic01 =
  ggplot(
    ...
    aes(
      x = income, 
      y = happiness)) +
  geom_point(shape = 16, size = 1, color = "lightblue") +
  geom_smooth(
    method = "lm",
    se = FALSE, 
    color = "#0BAFFF") +
...
# 预测happiness的值
predicted_happiness <- predict(income_linear_model, newdata = data.frame(income = income.dat$income))

# 使用ggplot2绘制预测值与真实值的关系图
df <- data.frame(income = income.dat$income, happiness = income.dat$happiness, predicted = predicted_happiness)
ggplot(df, aes(x = income, y = happiness)) +
  geom_point() +
  geom_line(aes(y = predicted), color = "blue") +
  labs(title = "Predicted vs Actual Happiness", x = "Income", y = "Happiness")
