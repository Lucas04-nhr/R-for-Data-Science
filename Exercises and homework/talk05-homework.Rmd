...
# 计算Time列的平均值
mean_time <- mean(Theoph$Time)

# 使用filter函数选择符合条件的行
filtered_data <- filter(Theoph, Dose > 5, Time > mean_time)
...
## 代码写这里，并运行；
# 计算Time列的平均值
mean_time <- mean(Theoph$Time, na.rm = TRUE)

# 使用mutate函数创建新列trend
Theoph <- mutate(Theoph, trend = Time - mean_time)

# 移除可能产生的NA值
Theoph <- na.omit(Theoph)
...
```{r}
# 使用mutate函数创建新列weight_cat
Theoph <- mutate(Theoph, weight_cat = 
  case_when(
